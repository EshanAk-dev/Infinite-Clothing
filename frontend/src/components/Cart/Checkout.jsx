...
const Checkout = () => {
  ...
  const handleCreateCheckout = async (e) => {
    ...
    if (cart && cart.products.length > 0) {
      ...
      if (res.payload && res.payload._id) {
        setCheckoutId(res.payload._id); // Set checkout id after checkout was successfull
      }
    }
  };
  ...
  const handlePaymentSuccess = async (details) => {
    try {
      ...
      await handleFinalizeCheckout(checkoutId); // Finalized order if checkout is successful
    } catch (error) {
      console.error(error);
    }
  };
    ...
    <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 max-w-7xl mx-auto py-10 px-6 tracking-tighter">
      ...
      <div className="bg-gray-50 p-6 rounded-lg">
        ...
        <div className="flex justify-between items-center text-lg mt-4 border-t pt-4">
          <p>Total</p>
          <p>Rs.{cart.totalPrice.toFixed(2)?.toLocaleString()}</p>
        </div>
      </div>
    </div>
  );
