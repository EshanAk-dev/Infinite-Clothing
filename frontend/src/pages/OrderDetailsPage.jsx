  ...
  if (!orderDetails) return (
    <div className="max-w-7xl mx-auto p-4 sm:p-6">
      <div className="bg-white rounded-xl shadow-sm p-8 text-center border border-gray-200">
        ...
        <p className="mt-1 text-sm text-gray-500">We couldn't find details for this order</p>
        <Link 
          to="/my-orders" 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-1 text-sm">
                <p><span className="text-gray-500">Method:</span> {orderDetails.paymentMethod}</p>
                <p><span className="text-gray-500">Status:</span> {orderDetails.isPaid ? 'Paid' : 'Pending'}</p>
                {orderDetails.paidAt && (
                  <p><span className="text-gray-500">Paid on:</span> {new Date(orderDetails.paidAt).toLocaleDateString()}</p>
                )}
              </div>
            </div>
            ...
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="space-y-1 text-sm">
                ...
                {orderDetails.deliveredAt && (
                  <p><span className="text-gray-500">Delivered on:</span> {new Date(orderDetails.deliveredAt).toLocaleDateString()}</p>
                )}
              </div>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <div className="w-full max-w-md space-y-2">
              <div className="flex justify-between pt-2">
                ...
                <span className="text-lg font-bold">Rs. {orderDetails.totalPrice.toFixed(2)}</span>
              </div>
            </div>
          </div>
        </div>
